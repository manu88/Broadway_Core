...
void CircleWaitComponent::prepareRessources( )
{
    ...
    m_rad = ( 2 * M_PI ) / m_segments;
    m_offset = ( 2 * M_PI ) / ( m_segments*2 );
...
void MousePointer::prepareRessources()
{
    setBounds(0, 0, 50, 50);
    setLayer( 10 );
...
void MousePointer::paint( const GXRect &rect )
{
    m_path->setFillColor(makeColor(0, 255, 255));
    m_path->addRect( getBounds() );
    m_path->stroke();
}



void MousePointer::deleteRessources()
{
    
}

void MousePointer::changed()
{
